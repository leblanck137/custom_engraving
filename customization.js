// Global variables to store the uploaded image and the sizes of the text and the uploaded image
var uploadedImage = null;
var uploadedImageSize = 0.4;  // Default size of the uploaded image
var uploadedImageX = 0.25; // Default horizontal position of the uploaded image
...
function handleClickOnCanvas(event) {
  ...
  // Determine the source canvas and adjust x, y coordinates accordingly
  var x, y;
  var sourceCanvas = text.some(textObj => textObj.toggleSwitchState) || images.some(imageObj => imageObj.toggleSwitchState)
    ? secondLargeCanvas
    : largeCanvas;
...
text.forEach(function(textObj, index) {
  ...
  if (x >= currentCanvas.width * textX[index] && x <= currentCanvas.width * (textX[index] + textWidth[index]) &&
      y >= currentCanvas.height / 2 - textHeight[index] / 2 && y <= currentCanvas.height / 2 + textHeight[index] / 2) {
...
console.log("Bounding rect for largeCanvas:", largeCanvas.getBoundingClientRect());
console.log("Bounding rect for secondLargeCanvas:", secondLargeCanvas.getBoundingClientRect());
...
images.forEach(function(imageObj, index) {
  ...
  var imageY = currentCanvas.height / 2 - imgHeight / 2;
    console.log('imageObj.element.width',imageObj.element.width);
      ...
      console.log('imgObj.x',imgObj);
      console.log('currentCanvas.width',currentCanvas.width);
      console.log('imageY',imageY);
      console.log('canvas: x >= ',largeCanvas.width * imageObj.x,' && x <= ',largeCanvas.width * imageObj.x + imgWidth,' && y >= ',imageY,' && y <= ',imageY + imgHeight)
    if (x >= currentCanvas.width * imageObj.x - (imgWidth / 2) && x <= currentCanvas.width * imageObj.x + (imgWidth / 2) &&
        y >= imageY && y <= imageY + imgHeight) {
...
// Add click event listeners to both canvases
